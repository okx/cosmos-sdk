...
func (app *BaseApp) PrepareParallelTxs(cb abci.AsyncCallBack, txs [][]byte) {
	...
	func(app *BaseApp) EndParallelTxs()[][]
	byte{
	...
	func(app *BaseApp) DeliverTxWithCache(req
	abci.RequestDeliverTx) abci.ExecuteRes{
	...
	func(e ExecuteResult) GetResponse()
	abci.ResponseDeliverTx{
		return e.Resp
	}

	func(e ExecuteResult) Conflict(cache
	abci.AsyncCacheInterface) bool{
		rerun, := false
		if e.Ms == nil{
	...
	func(e ExecuteResult) Collect(cache
	abci.AsyncCacheInterface) {
		if e.Ms == nil {
			return
		}
		e.Ms.IteratorCache(func(key, value []byte, isDirty bool) bool {
	...
	func(e ExecuteResult) GetCounter()
	uint32{
		return e.Counter
	}
	...
	func
	NewExecuteResult(r
	abci.ResponseDeliverTx, ms
	sdk.CacheMultiStore, counter
	uint32, evmCounter
	uint32) ExecuteResult{
	...
	func
	NewAsyncWorkGroup() * AsyncWorkGroup{
	...
	func(a *AsyncWorkGroup) Push(item
	ExecuteResult) {
		a.WorkCh <- item
	}
	...
	func
	newParallelTxManager() * parallelTxManager{
	...
	func(f *parallelTxManager) SetFee(key
	string, value
	sdk.Coins) {
		f.mu.Lock()
		defer f.mu.Unlock()
	...
	func(f *parallelTxManager) GetFeeMap()
	map[string]sdk.Coins{
		f.mu.RLock(),
		defer f.mu.RUnlock()
		return f.fee
	}
	func(f *parallelTxManager) SetRefundFee(key
	string, value
	sdk.Coins) {
		f.mu.Lock()
		defer f.mu.Unlock()
	...
	func(f *parallelTxManager) GetRefundFeeMap()
	map[string]sdk.Coins{
		f.mu.RLock(),
		defer f.mu.RUnlock()
		return f.refundFee
