...
func TestLoadStore(t *testing.T) {
	...
	hash, verHc, _, err := tree.SaveVersion()
...
func TestGetImmutable(t *testing.T) {
	...
	require.Panics(t, func() { newStore.Commit(&iavl.TreeDelta{}) })
}
...
func nextVersion(s *Store) {
	key := []byte(fmt.Sprintf("Key for tree: %d", s.LastCommitID().Version))
	value := []byte(fmt.Sprintf("Value for tree: %d", s.LastCommitID().Version))
	s.Set(key, value)
	s.Commit(&iavl.TreeDelta{})
}
...
func TestIAVLStoreQuery(t *testing.T) {
	...
	cid, _ := iavlStore.Commit(&iavl.TreeDelta{})
	ver := cid.Version
