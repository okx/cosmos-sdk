...
func PostCommands(cmds ...*cobra.Command) []*cobra.Command {
	for _, c := range cmds {
		...
		c.Flags().String(FlagFees, "10000000000neva", "Fees to pay along with transaction; eg: 10000000000neva")
		c.Flags().String(FlagGasPrices, "", "Gas prices to determine the transaction fee (e.g. 10neva)")
		c.Flags().String(FlagNode, "tcp://localhost:20181", "<host>:<port> to tendermint rpc interface for this chain")
		c.Flags().Bool(FlagUseLedger, false, "Use a connected Ledger device")
